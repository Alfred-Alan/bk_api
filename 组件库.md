...
def add_task(task_name, every: int, period: str, task_path: str, kwargs={}, args=[]):
    """
    添加任务
    :param task_name: string 任务名称
    :param every: int 间隔
    :param period: 周期 例：days,hours,minutes,seconds,microseconds
    :param task_path: 任务路径 例：'home_application.tasks.test'
    :return:
    """
    periods = {
    ...
    old_task = None
    try:
        old_task = PeriodicTask.objects.get(name=task_name)
        ...
        is_interval_schedule_exist = False
        try:
            interval_schedule = IntervalSchedule.objects.get(
                every=every,
                period=periods[period],
            )
            is_interval_schedule_exist = True
            ...
            try:
                if interval_schedule is None:
                    logger.error("interval调度器创建失败")
                if not is_interval_schedule_exist:
                ...
                logger.error(f"添加设备 任务失败: {repr(e)}")
                print("添加任务失败")
```
...
<script type="text/javascript">
	downloadUrl: site_url + "test_download"
</script>
```

```python
data_json = [{"id": "1", "name": "www"}]
def download_template(request):
    save_csv("test",data_json)

    file = open('test.csv', 'rb')
    response = FileResponse(file)
    response['Content-Type'] = 'application/octet-stream'
    response['Content-Disposition'] = 'attachment;filename="ExampleTemplate.xlsx"'
    return response
